...
struct SmokeWidget : ModuleWidget {
  ...
  void setPanelToMode() {
    if (currentPanelMode == clouds::PLAYBACK_MODE_SPECTRAL)
      setPanel(SVG::load(assetPlugin(plugin, "res/Espectro-parasite.svg")));
    else if (currentPanelMode == clouds::PLAYBACK_MODE_LOOPING_DELAY)
      setPanel(SVG::load(assetPlugin(plugin, "res/Ritardo-parasite.svg")));
    else if (currentPanelMode == clouds::PLAYBACK_MODE_STRETCH)
      setPanel(SVG::load(assetPlugin(plugin, "res/Camilla-parasite.svg")));
    else if (currentPanelMode == clouds::PLAYBACK_MODE_OLIVERB)
      setPanel(SVG::load(assetPlugin(plugin, "res/Oliverb.svg")));
    else if (currentPanelMode == clouds::PLAYBACK_MODE_RESONESTOR)
      setPanel(SVG::load(assetPlugin(plugin, "res/Resonestor.svg")));
    else
      setPanel(SVG::load(assetPlugin(plugin, "res/Smoke-parasite.svg")));    
  }
...
struct Oliverb : Smoke {
  Oliverb() : Smoke() {
    playbackmode = clouds::PLAYBACK_MODE_OLIVERB;
  }
};

struct Resonestor : Smoke {
  Resonestor() : Smoke() {
    playbackmode = clouds::PLAYBACK_MODE_RESONESTOR;
  }
};

Model *modelSmoke 	= Model::create<Smoke,SmokeWidget>( 	 "Southpole Parasites", "Smoke", 		"Smoke", GRANULAR_TAG, REVERB_TAG);
Model *modelEspectro   = Model::create<Espectro,SmokeWidget>(    "Southpole Parasites", "Espectro",    "Espectro", GRANULAR_TAG, REVERB_TAG);
Model *modelRitardo   = Model::create<Ritardo,SmokeWidget>(    "Southpole Parasites", "Ritardo",    "Ritardo", GRANULAR_TAG, REVERB_TAG);
Model *modelCamilla   = Model::create<Camilla,SmokeWidget>(    "Southpole Parasites", "Camilla",    "Camilla", GRANULAR_TAG, REVERB_TAG);
Model *modelOliverb   = Model::create<Oliverb,SmokeWidget>(    "Southpole Parasites", "Oliverb",    "Oliverb", GRANULAR_TAG, REVERB_TAG);
Model *modelResonestor   = Model::create<Resonestor,SmokeWidget>(    "Southpole Parasites", "Resonestor",    "Resonestor", GRANULAR_TAG, REVERB_TAG);
